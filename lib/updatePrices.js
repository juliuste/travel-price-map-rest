...
const pricesDB = (origin, destination) => {
    ...
    return Promise.all(promised).then((results) => {
        ...
        const shopLink = `https://bahn.guru/calendar?from=${origin.ids.db}&to=${destination.ids.db}&submit=↳&class=2&bc=0&start=&end=&duration=&weeks=5`
